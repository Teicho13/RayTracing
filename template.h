...
static const char* TemplateVersion = "Retake Tracer";

//SIMD Intrinsics headers.
//#include "emmintrin.h"
//#include "immintrin.h"

inline float Rand() { return (static_cast<float>(rand()) / (RAND_MAX + 1)); }
inline float Rand(float range) { return (static_cast<float>(rand()) / RAND_MAX) * range; }
inline float Rand(float min, float max) { return (min + (max - min) * Rand()); }
inline int IRand(int range) { return rand() % range; }
inline int IRand(float range, float range2) { return static_cast<int>(Rand(range, range2)); }
...
namespace Tmpl8 {
	...
	*/
	...
	inline float degrees_to_radiansf(float degrees) {
